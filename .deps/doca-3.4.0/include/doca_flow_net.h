...
#define DOCA_FLOW_PROTO_IPV6_ROUTE (43)		/**< IPv6 Routing Header Protocol. */
...
/**
 * @brief doca flow IPv6 address
 */
struct doca_flow_ipv6_addr {
	doca_be32_t addr[4];
	/**< IPv6 address as four 32-bit big-endian words. */
};

/**
 * @brief doca flow IPv6 Segment Routing Header (SRH) base fields
 *
 * Represents the fixed 8-byte base header of the SRH as defined by RFC 8754.
 */
struct doca_flow_header_ipv6_srh_base {
	uint8_t next_header;
	/**< Next header type. */
	uint8_t hdr_ext_len;
	/**< Extension header length (in 8-octet units,
	     excluding the first 8 octets). */
	uint8_t routing_type;
	/**< Routing type (fixed to 4 for SRv6). */
	uint8_t segments_left;
	/**< Number of remaining segments. */
	uint8_t last_entry;
	/**< Index of last element in Segment List. */
	uint8_t flags;
	/**< Flags as defined for the SRv6 header. */
	doca_be16_t tag;
	/**< Tag, as defined by RFC 8754. */
};

/**
 * @brief doca flow IPv6 Segment Routing Header (SRH) with segment list
 *
 * The segment list follows the 8-byte base header.  Each segment is a
 * 128-bit IPv6 address.
 */
struct doca_flow_header_ipv6_srh {
	struct doca_flow_header_ipv6_srh_base base;
	/**< SRH base header (8 bytes). */
	struct doca_flow_ipv6_addr segments[];
	/**< Flexible array of IPv6 segment addresses. */
};
...
struct doca_flow_tun {
	...
	union {
		...
		struct {
			...
			/**< gtp PSC extension: reserved (2) + QoS flow identifier (6) */
		};
