...
typedef enum {
   ...
   Default timestamp that is selected by the device. Deprecated in this release; will be removed
...
/**
 * @brief Set the log file path for PCC logging
 *
 * Must be set before calling doca_pcc_start()
 * @note If not set by this function, stdout is used by default
 *
 * @param[in] pcc - PCC context
 * @param[in] file_path - path to the PCC log file
 *
 * @return
 * - DOCA_SUCCESS - in case of success
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_pcc_log_file_set(struct doca_pcc *pcc, const char *file_path);

/**
 * @brief Get the log file path used for PCC logging
 *
 * @param[in] pcc - PCC context
 * @param[out] file_path - buffer to receive the PCC log file path
 * @param[out] file_path_len - length of the file_path buffer; output is the actual number of bytes written
 *
 * The file_path buffer is allocated by the caller, and file_path_len must be set to the allocated length.
 * Upon return, file_path_len is set to the actual length of file_path.
 *
 * @return
 * - DOCA_SUCCESS - in case of success
 * - DOCA_ERROR_INVALID_VALUE - received invalid null input
 * - DOCA_ERROR_BAD_CONFIG - log file was not previously set
 */
DOCA_EXPERIMENTAL
doca_error_t doca_pcc_log_file_get(struct doca_pcc *pcc, char **file_path, uint32_t *file_path_len);
 ...
 * - DOCA_SUCCESS - in case of success
 * - DOCA_ERROR_INVALID_VALUE - received invalid null input
 * - DOCA_ERROR_BAD_CONFIG - trace file was not previously set
 */
DOCA_EXPERIMENTAL
...
/**
 * @brief Get number of library internal threads
 *
 * @param[in]  pcc - PCC context
 * @param[out]  num_threads - Number of library internal threads
 *
 * @return
 * DOCA_SUCCESS - in case of success
 * doca_error code - in case of failure:
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_pcc_get_internal_num_threads(const struct doca_pcc *pcc, uint32_t *num_threads);

/**
 * @brief Set thread affinity configuration for library internal threads
 *
 * This function configures the thread affinity for library internal threads that will be created
 * on the DPA. Each thread can be pinned to a specific hardware thread (EU) ID.
 * Must be set before calling doca_pcc_start() if doca_pcc_get_internal_num_threads() returns a non-zero value.
 *
 * @param[in]  pcc - PCC context
 * @param[in]  num_threads - Number of library internal threads
 * @param[in]  affinity_configs - Array of hart indexes to assign to threads
 *
 * @return
 * DOCA_SUCCESS - in case of success
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_pcc_set_internal_thread_affinity(struct doca_pcc *pcc,
						   uint32_t num_threads,
						   uint32_t *affinity_configs);
#ifdef __cplusplus
}
