...
An egress gate is created lazily per `(vSwitch, physical DPDK port)` membership.
It implements split horizon from the low 16 bits of `pkt_meta`, so
...
membership egress-gate pipes  M
egress allow/forward entries  T
