...
void eswitch_cli_help(FILE *output, const char *program,
  ...
  fprintf(output,
          ...
          "  vr egress policy set --id <id> --interface <guest-rif>\n"
          "      --default allow|deny                Set guest-network default\n"
          "  vr egress policy show|delete --id <id> --interface <guest-rif>\n"
          "  vr egress rule add --id <id> --interface <guest-rif>\n"
          "      --rule-id <id> --action allow|deny --protocol tcp|udp|icmp|all\n"
          "      [--source <cidr>] [--destination <cidr>]\n"
          "      [--port-range <port|first-last>] [--icmp-type <0-255>]\n"
          "      [--icmp-code <0-255>]\n"
          "  vr egress rule show --id <id> --interface <guest-rif>\n"
          "      [--rule-id <id>]\n"
          "  vr egress rule delete --id <id> --interface <guest-rif>\n"
          "      --rule-id <id>\n\n"
          "  vr port-forward add --id <id> --rule-id <id> --interface <name>\n"
          "      --protocol tcp|udp --public-port <port|first-last>\n"
          "      --private-ip <ip> --private-port <port|first-last>\n"
