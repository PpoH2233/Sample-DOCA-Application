#ifndef ESWITCH_CONFIG_H
#define ESWITCH_CONFIG_H

#include <stdint.h>

#define ESWITCH_SOCKET_PATH "/run/eswitch-management/control.sock"
#define ESWITCH_STATE_PATH "/var/lib/eswitch-management/eswitch.conf"
#define ESWITCH_MAX_VSWITCHES 64U
#define ESWITCH_MAX_VLAN_MEMBERSHIPS 512U
/* Persistence and the runtime pipeline must enforce the same limit.  Apart
 * from rejecting an unrestorable configuration, a larger persistence-only
 * table makes every ARM slow-path membership lookup scan thousands of slots. */
#define ESWITCH_MAX_PERSISTED_MEMBERS ESWITCH_MAX_VLAN_MEMBERSHIPS
#define ESWITCH_RESPONSE_SIZE (128U * 1024U)
#define ESWITCH_HW_ROUTE_DEFAULT_CAPACITY 1024U
#define ESWITCH_HW_ROUTE_MIN_CAPACITY 64U
#define ESWITCH_HW_ACTIONS_MEM_MIN_SIZE (512U * 1024U)
#define ESWITCH_SF_ACTION_ENTRIES_PER_CONTEXT 2U
/* DOCA's flow_acl sample budgets 32 action-memory units per ACL entry. */
#define ESWITCH_ACL_ACTION_MEM_UNITS_PER_ENTRY 32U
#define ESWITCH_ACL_ACTION_DEFAULT_ENTRIES 16U
#define ESWITCH_ACL_ACTION_MAX_ENTRIES 512U
#define ESWITCH_HW_RETRY_INITIAL_NS UINT64_C(1000000000)
#define ESWITCH_HW_RETRY_MAX_NS UINT64_C(60000000000)
#define ESWITCH_UPLINK_ARP_DEFAULT_PPS 256U
#define ESWITCH_UPLINK_ARP_DEFAULT_BURST 64U
#define ESWITCH_UPLINK_ARP_MAX_PPS 1000000U
#define ESWITCH_CT_DEFAULT_CAPACITY 4096U
#define ESWITCH_CT_MIN_CAPACITY 64U
#define ESWITCH_CT_MAX_CAPACITY 4096U
#define ESWITCH_CT_ACTIONS_MEM_PER_CONNECTION 256U
#define ESWITCH_CT_ACTIONS_MEM_MIN_SIZE (512U * 1024U)
#define ESWITCH_CT_QUEUE_DEPTH 512U

/* 0 means "not assigned" and is never a valid virtual-switch ID. */
#define ESWITCH_MIN_VSWITCH_ID 1U
#define ESWITCH_MAX_VSWITCH_ID UINT16_MAX

#endif /* ESWITCH_CONFIG_H */
