...
int main(void) {
  ...
  assert(table->count==2); /* The UDP session expires before TCP sessions. */
  router_nat_age(table,ROUTER_NAT_TCP_IDLE_NS+ROUTER_NAT_UDP_IDLE_NS+20);
  assert(table->count==0 && table->stats.sessions_aged==3);
  free(table);
  puts("PASS: NAT policy-independent TCP/UDP SNAT, reverse lookup, checksums, isolation and aging");
  return 0;
}
