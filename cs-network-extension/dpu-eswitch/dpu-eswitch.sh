...
##############################################################################
# dpu-eswitch.sh  (extension: dpu-eswitch)
#
# Proxy script for the dpu-eswitch CloudStack NetworkOrchestrator extension.
# Runs on the CloudStack management server.
#
# Invocation model:
#   dpu-eswitch.sh <command> <payload-file> <timeout-seconds>
#
...
#
# Two runtime modes:
#  1) ensure-network-device (local, no SSH): selects/revalidates the BlueField
#     Arm host and emits a single-line JSON object like:
#       {"host":"192.0.2.10","vswitch_id":"100","vr_id":"100","vlan":"100"}
#  2) all other commands: forwards the payload file to the selected host and
#     executes dpu-eswitch-wrapper.sh remotely (device side).
#
...
#   DPU Arm (wrapper): /etc/cloudstack/extensions/<name>/<name>-wrapper.sh
#
...
#   CS_NET_SCRIPT_PATH=/custom/path/wrapper.sh dpu-eswitch.sh <cmd> ...
# ---------------------------------------------------------------------------
...
# Remote wrapper path on each BlueField Arm host.
...
if [ $# -lt 1 ]; then
    die "Usage: dpu-eswitch.sh <command> <payload-file> <timeout-seconds>" 1
fi
...
if [ "${PAYLOAD_MODE}" = "true" ]; then
    ...
    if [ "${COMMAND}" = "custom-action" ]; then
        ...
        VLAN=$(payload_json_get "${PAYLOAD_FILE}" "payload.vlan")
...
if [ "${COMMAND}" = "ensure-network-device" ]; then
    if [ -n "${VPC_ID}" ]; then
        die "ensure-network-device: VPC networks are not supported by ${_EXT_DIR_NAME} (isolated only)" 1
    fi
    ...
    # Network identity on the DPU: vswitch id = virtual router id = guest VLAN.
    # When CloudStack has not assigned a VLAN yet the ids are omitted and the
    ...
    # This preserves the host-DPU binding across API calls once a network
    # has been implemented on a particular BlueField Arm host.
        ...
        for h in "${HOST_LIST[@]}"; do
            ...
            if [ "${h}" = "${CURRENT_HOST}" ]; then
                if host_reachable "${CURRENT_HOST}"; then
                    ...
                    if [ -n "${VLAN}" ]; then
                        printf '{"host":"%s","vswitch_id":"%s","vr_id":"%s","vlan":"%s"}\n' \
                            "${CURRENT_HOST}" "${VLAN}" "${VLAN}" "${VLAN}"
                    else
                        printf '{"host":"%s"}\n' "${CURRENT_HOST}"
                    fi
                    exit 0
    ...
    #
    # Each KVM host carries exactly one DPU; the network is pinned to one DPU.
    # The NETWORK_ID is the routing key: CRC32 (via cksum) modulo the host
    # count gives a stable preferred index, and hosts are probed from that
    # index, wrapping around, until a reachable one is found.
    _ROUTE_KEY="${NETWORK_ID}"
